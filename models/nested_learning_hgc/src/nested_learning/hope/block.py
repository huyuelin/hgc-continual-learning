...
from ..gradient_memory import LevelGradientMemory
...
class HOPEBlock(nn.Module):
    def __init__(self, config: HOPEBlockConfig):
        ...
        # HGC: create gradient memory if enabled
        grad_memory = None
        if config.hgc_enabled:
            grad_memory = LevelGradientMemory(
                specs,
                r_base=config.hgc_r_base,
                gamma_scale=config.hgc_gamma_scale,
                distillation_strength=config.hgc_distillation_strength,
            )
