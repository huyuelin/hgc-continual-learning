...
class ModelConfig:
    ...
    # HGC (Hierarchical Gradient Consolidation) settings
    hgc_enabled: bool = False
    hgc_r_base: int = 4
    hgc_gamma_scale: float = 0.1
    hgc_distillation_strength: float = 0.05
...
class HOPEModel(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        if variant == "hope_attention":
            ...
            hybrid_block_config = HOPEBlockConfig(
                ...
                optimizer_configs=config.optimizers or {},
                hgc_enabled=config.hgc_enabled,
                hgc_r_base=config.hgc_r_base,
                hgc_gamma_scale=config.hgc_gamma_scale,
                hgc_distillation_strength=config.hgc_distillation_strength,
            )
            self.blocks = nn.ModuleList(
    ...
    def consolidate_gradient_memory(self) -> None:
        """Consolidate gradient signatures at task/segment boundaries (HGC).

        Call this at the end of each task or data segment to:
        1. Extract gradient signatures from accumulated buffers
        2. Update cross-level distillation bases
        """
        for block in self.blocks:
            grad_mem = getattr(block, "grad_memory", None)
            if grad_mem is not None:
                grad_mem.consolidate_all()

    def hgc_diagnostics(self) -> dict:
        """Return HGC diagnostic stats for all blocks."""
        diagnostics = {}
        for i, block in enumerate(self.blocks):
            grad_mem = getattr(block, "grad_memory", None)
            if grad_mem is not None:
                diagnostics[f"block_{i}"] = grad_mem.signature_stats()
        return diagnostics
